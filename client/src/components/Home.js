...
export default function Home() {
  ...
  return (
    <>
      <div className=" bg-slate-100 mt-[-10px]">
        ...
        <div className="home-category bg-white rounded-xl border-2 border-black-200 m-2 w-[22%] h-[100%] min-w-[250px] relative">
            <div
              className="pl-[10px] pb-[10px] pt-[15px] pr-[25px] text-[1rem] hover:bg-slate-100"
            ...
            {showItem1Items && (
              <div
                className="absolute bg-white rounded-xl border-2 border-black-200 pl-[10px] pb-[10px] pt-[15px] pr-[25px] text-[1rem]"
                onMouseEnter={handleItem1HoverEnter}
                onMouseLeave={handleItemHoverLeave}
                style={{
                  zIndex: 20,
                ...
                <div className="w-[15rem]">
                  {[...Array(5)].map((_, index) => (
                    <div
                      key={index}
                      className="pl-[10px] pb-[10px] pt-[15px] pr-[25px] text-[1rem] hover:bg-slate-100"
                      onMouseEnter={handleAdditionalItemHoverEnter} // Use the same hover handler for all additional items
                      onMouseLeave={handleItemHoverLeave}
                      style={{ top: `${index * 40}px` }} // Adjust the height dynamically
                    >
                      <p>Additional Item {index + 1}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {showOtherItems && (
              <div
                className="absolute bg-white rounded-xl border-2 border-black-200 pl-[10px] pb-[10px] pt-[15px] pr-[25px] text-[1rem]"
                onMouseEnter={handleAdditionalItemHoverEnter}
                onMouseLeave={handleItemHoverLeave}
                style={{
                  zIndex: 20,
                  maxWidth: "250px",
                ...
                <div className=" flex flex-col justify-between gap-0 ">
                  ...
                  <div className="grid grid-cols-5">
                    {home_data.map((data) => (
                      <button
                        key={data}
                        className=" rounded-sm home-discount border-r-2 p-2  pr-4 "
                        onClick={() => handleIndicatorClick(data.num)}
                        onMouseEnter={() => setActiveIndex(data.num)}
                      >
                        {data.name}
                        <br />
                        {data.offer}
